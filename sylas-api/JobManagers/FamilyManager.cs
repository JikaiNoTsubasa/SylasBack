...
public class FamilyManager(SyContext context) : SyManager(context)
{
    ...
    private IQueryable<FamilyTask> GetTasks(long? memberId = null)
    {
    ...
    public FamilyTask FetchTask(long id, long? memberId = null)
    {
        return GetTasks(memberId).FirstOrDefault(t => t.Id == id)
    ...
    public (FamilyTask task, FamilyMember member) CompleteTask(long taskId, long memberId)
    {
        ...
        _context.SaveChanges();
        return (FetchTask(taskId, memberId), FetchMember(memberId));
    }
